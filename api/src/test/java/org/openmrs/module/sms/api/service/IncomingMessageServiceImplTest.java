...
public class IncomingMessageServiceImplTest  {
	...
	public void setUp() {
		...
		given(administrationService.getGlobalProperty(SMSConstants.GP_DISABLE_INCOMING_MESSAGE_HANDLERS))
	...
	public void testHandleMessageSafe() throws Exception {
		...
		IncomingMessageData incomingMessageData = new IncomingMessageDataBuilder().setConfig(config).build();
		...
		verify(administrationService).getGlobalProperty(SMSConstants.GP_DISABLE_INCOMING_MESSAGE_HANDLERS);
	}
